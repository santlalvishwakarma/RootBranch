...
public class ProductDefinitionAddEditBB extends BackingBean {
	...
	private ProductHierarchyDVO selectedProductHierarchyRecord;
	...
	private Map<Long, ProcessDVO> processActiveMap;
	private ProductHierarchyMappingDVO productHierarchyRecord;
	private Map<Long, BaseDVO> copyOfDataMap;
	private Map<Integer, ProductCategoryLevelDVO> levelNameMap;
	private List<Object> propertyValuesList;

	private boolean validateForApprove;
	private ProductItemPropertyMappingDVO selectedItemCategory;
	...
	private void populateEnableDisableButtons() {
		...
		Integer productVersion = productOpr.getProductRecord().getProductVersion();
		Integer skuVersion = productOpr.getProductRecord().getProductSkuRecord().getSkuVersion();
		...
		if (renderForProduct) {
			...
			if (productVersion != null && productVersion > 1) {
				disableFields = true;
			}
			...
			if (skuVersion != null && skuVersion > 1) {
				disableFields = true;
			}
		}
	...
	public void openMapHierarchyDialog(ActionEvent event) {
		...
		try {
			...
			hierarchyListForAutoSuggest = new ProductDefinitionBD().getSuggestedHierarchies(new ProductHierarchyDVO());
			FacesContext.getCurrentInstance().getViewRoot().getViewMap()
