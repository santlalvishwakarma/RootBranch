package com.web.bb.common.login;
	...
	public void performLogin(ActionEvent event) {
		...
		if (validateLogin()) {
			try {
				...
				putObjectInCache(CommonConstant.LOGGED_USER_KEY, loginOpr.getUserDetails().getUserLogin());

				putObjectInCache(CommonConstant.LOGGED_USER_DATA, loginOpr.getUserDetails());
				...
				for (int i = 0; i < loginOpr.getUserDetails().getUserRolesMappingList().size();) {
					putObjectInCache(CommonConstant.LOGGED_USER_ROLES, loginOpr.getUserDetails()
