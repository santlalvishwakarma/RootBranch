...
IF p_product_id IS NOT NULL AND p_product_sku_id IS NULL THEN	
	...
	SELECT 	'HDR', ph.product_id, ph.product_code, ph.product_name, ph.product_description,
			...
			fn_product_get_product_sku_image(p_product_id, p_product_sku_id) AS image_url
	FROM 	product_header ph
	WHERE 	ph.product_id =  p_product_id;
	
ELSEIF p_product_sku_id IS NOT NULL THEN
	
	SELECT 	'HDR',psh.product_id, psh.product_sku_id, psh.sku_code, psh.sku_name, psh.sku_description, psh.sku_version, psh.sku_comments,
			psh.status_code AS sku_status_code, fn_core_get_status_name(psh.status_code) AS  sku_status_name, psh.is_active AS sku_is_active, psh.modified_date,
			ph.product_code, ph.product_name, ph.product_description,
			ph.status_code, fn_core_get_status_name(ph.status_code) AS status_name,
			fn_product_get_product_sku_image(p_product_id, p_product_sku_id) AS image_url
